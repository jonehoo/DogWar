...
module.exports =
    (function Utils() {
        ...
        function updateWxRankTexture(){
            ...
            cc.WxRankBoard.getComponent(cc.Sprite).spriteFrame = new cc.SpriteFrame(cc.WxRankTexture);
        }
        ...
        function reportRank(data){
            ...
            this.ensurePrivacyAuthorize(function(ok){
                if(!ok){
        ...
        function showWxRank(data){
            if(!window.wx || !cc.MyPlat){
                this.showToast('仅支持微信小游戏排行榜');
                return;
            }

            if(!this.canUseWxOpenDataRank()){
                this.showToast('未配置开放数据域排行榜');
                return;
            }
            ...
            this.ensurePrivacyAuthorize(function(ok){
                if(!ok){
                    self.showToast('需同意隐私协议才能查看排行榜');
                    return;
                }

                var layer = self.buildRankLayer();
                if(!layer){
                    self.showToast('排行榜加载失败');
                    return;
                }
